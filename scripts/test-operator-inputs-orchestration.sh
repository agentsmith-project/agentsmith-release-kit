...
deployment_path_dir() {
  local package_dir="$1"
  printf '%s\n' "$package_dir/.release-kit-internal/online-use-existing/deployment-path"
}
...
expect_fail_matching dry_run_run 'currently supports only online/use_existing with mode apply' \
...
expect_fail_matching airgap_install_run 'currently supports only online/use_existing with mode apply' \
