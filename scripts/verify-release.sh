...
Bootstrap status:
  ...
  --release-engineering-gate-intake is maintainer-only for explicit GA or compliance trigger work; it consumes the focused online adoption report plus focused airgap/use_existing and airgap/install_substrates adoption reports, writes readiness=false and formal_verdict=not_issued only, and is not deploy, package, offline, operator verdict, or release readiness.
