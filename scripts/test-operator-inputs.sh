...
switch (caseName) {
  ...
  case 'operator_facing_airgap_bundle_manifest': {
    const bundleManifestPath = path.join(
      path.dirname(manifestPath),
      manifest.airgap_bundle_manifest
    );
    const bundleManifest = JSON.parse(fs.readFileSync(bundleManifestPath, 'utf8'));
    delete bundleManifest.target_profile;
    delete bundleManifest.substrate;
    fs.writeFileSync(bundleManifestPath, `${JSON.stringify(bundleManifest, null, 2)}\n`);
    break;
  }
...
example_online_package="$TMP_DIR/example-online-use-existing"
...
"$NODE_BIN" --input-type=module - "$doctor_static_secret_ref_payload_dir/render-values.example.json" <<'NODE'
import fs from 'node:fs';

const [renderValuesPath] = process.argv.slice(2);
const renderValues = JSON.parse(fs.readFileSync(renderValuesPath, 'utf8'));
renderValues.kubernetes_secret_ref_payload = `
apiVersion: v1
kind: Pod
spec:
  containers:
        ...
        - name: APP_PASSWORD
          valueFrom:
            secretKeyRef:
              name: app-secret
              key: password
              password: rawsecretvalue
`;
fs.writeFileSync(renderValuesPath, `${JSON.stringify(renderValues, null, 2)}\n`);
NODE
if "$NODE_BIN" "$ROOT_DIR/scripts/resolve-operator-inputs.mjs" \
  --operator-inputs "$doctor_static_secret_ref_payload_dir" \
  --doctor \
  --stdout >"$TMP_DIR/doctor-static-kubernetes-secret-ref-payload.json"; then
  fail "operator-inputs doctor should fail when Secret reference block contains raw secret-like fields"
fi
"$NODE_BIN" --input-type=module - "$TMP_DIR/doctor-static-kubernetes-secret-ref-payload.json" <<'NODE'
import fs from 'node:fs';

const report = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));
const issue = report.static_issues.find((item) => item.field === 'render_values');
if (!issue || !/secret-looking value/.test(issue.reason)) {
  throw new Error('doctor static Secret reference blocker must explain raw secret-like payload');
}
NODE
pass "operator-inputs doctor rejects raw secret-like fields inside Kubernetes Secret reference blocks"
...
operator_facing_airgap_bundle_dir="$TMP_DIR/operator-facing-airgap-bundle"
copy_valid_package "$base_airgap" "$operator_facing_airgap_bundle_dir"
mutate_manifest "$operator_facing_airgap_bundle_dir" operator_facing_airgap_bundle_manifest
"$NODE_BIN" "$ROOT_DIR/scripts/resolve-operator-inputs.mjs" \
  --operator-inputs "$operator_facing_airgap_bundle_dir" >/dev/null
assert_plan "$operator_facing_airgap_bundle_dir/.release-kit-internal/operator-inputs-plan.json" airgap/use_existing
pass "resolve-operator-inputs derives airgap bundle manifest target/substrate identity from deployment_path"
...
mutate_manifest "$operator_facing_airgap_install_dir" operator_facing_airgap_bundle_manifest
"$NODE_BIN" "$ROOT_DIR/scripts/resolve-operator-inputs.mjs" \
