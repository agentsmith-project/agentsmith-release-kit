...
| R-012 | Airgap bundle manifest/digest diagnostics could be mistaken for package creation or offline install readiness. | Open | `--airgap-bundle-check` writes `readiness: false`, uses `scope: airgap_bundle_manifest_check_only`, accepts only `existing_kubernetes/external_declared/airgap`, checks only local manifest paths plus deploy template archive and declared file sha256 values, and docs state it does not parse the `.tgz`, create a package, call Docker, skopeo, oras, kubectl, pull, push, mirror, save, or load images. |
